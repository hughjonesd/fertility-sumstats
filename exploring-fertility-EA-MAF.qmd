...
Within each group, I estimated $\sigma^2_X+\sigma^2_\eta$ by the variance of the group's estimated EA4 effect sizes, and $\sigma^2_\eta$ from the square of the mean reported standard error of the EA4 effect size estimates (after adjustment for stratification).[^1] I then calculated the corrected $\beta$ for each group using the formula above.
...
ggplot(group_regs, aes(mean_maf, beta)) +
  geom_point(size = 0.8, alpha = 0.8) +
...
plot_sim <- function(result, true_relationship) {
  ggplot(result, aes(mean_maf, beta)) + 
  geom_point(aes(y = beta_hat), size = 0.5, alpha = 0.7, color = "grey80") +
  geom_point(size = 0.5, alpha = 0.7) + 
  geom_smooth(method = "loess", formula = y ~ x) + 
  geom_smooth(method = "loess", formula = y ~ x, aes(y = beta_hat), 
              color = "darkgrey") + 
