...
nice_n <- function (x) prettyNum(x, scientific = FALSE, 
                                 big.mark = ",")
```
...
# I'll use a big sample to show that the estimates are indeed
...
ggplot(pop, aes(rlrs)) + 
  ...
  theme_light() + 
  labs(x = "RLRS", y= "Proportion of population")
...
```{r plot-true-pgs}
#| fig-cap: !expr paste("Individual true PGS against Relative Lifetime Reproductive Success,", nice_n(n), "simulated individuals and linear regression line")
ggplot(pop, aes(pgs_true, rlrs)) + 
  geom_point(alpha = 0.05, pch = ".", 
             position = position_jitter(height = 0.2)) +
  geom_smooth(method = "lm", formula = y~x, color = "red") +
  coord_cartesian(ylim = c(0, 3)) + 
  labs(x = "True PGS", y = "RLRS")
```
...
Compare the plot of measured PGS $p$ against RLRS to the previous plot, which used the true PGS $p^*$:

```{r plot-measured-pgs}
#| fig-cap: !expr paste("Individual measured PGS against Relative Lifetime Reproductive Success,", nice_n(n), "simulated individuals and linear regression line")

ggplot(pop, aes(pgs, rlrs)) + 
  geom_point(alpha = 0.05, pch = ".", 
             position = position_jitter(height = 0.2)) +
  geom_smooth(method = "lm", formula = y~x, color = "darkgreen") +
  coord_cartesian(ylim = c(0, 3)) + 
  labs(x = "Measured PGS", y = "RLRS")

```

## Correcting our estimates

But what we really want to know is the covariance of the true PGS with RLRS. That will give us the average true PGS in the next generation.
...
I thought this would be useful for anyone who is a statistical schmo like me. For a more rigorous treatment, see e.g. the appendix to Beauchamp (2016); or Becker et al. (2021) [here](https://research.vu.nl/ws/portalfiles/portal/152631985/Resource_profile_and_user_guide_of_the_Polygenic_Index_Repository.pdf).
